...
let elementFinder: ElementFinder = element(By.css('.e-tab'));
    ...
    it('Default', async (done: DoneFn) => {
        Helper.getAndWait(Helper.tsUrl + 'tab/default.html', Helper.tab);
    ...
    it('Orientation', async (done: DoneFn) => {
        Helper.getAndWait(Helper.tsUrl + 'tab/orientation.html', Helper.tab);
        await element(By.css('.e-tab')).isDisplayed().then((flag: boolean) => {
            expect(flag).toEqual(true);
        });
        browser.sleep(1000);
    ...
    it('Responisve', async (done: DoneFn) => {
        Helper.getAndWait(Helper.tsUrl + 'tab/responsive-modes.html', Helper.tab);
        await element(By.css('.e-tab')).isDisplayed().then((flag: boolean) => {
            expect(flag).toEqual(true);
        });
        browser.sleep(1000);
    ...
    it('Wizard', async (done: DoneFn) => {
        Helper.getAndWait(Helper.tsUrl + 'tab/wizard.html', Helper.tab);
        await element(By.css('.e-tab')).isDisplayed().then((flag: boolean) => {
            expect(flag).toEqual(true);
        });
        browser.sleep(1000);
