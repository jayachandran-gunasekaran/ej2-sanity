...
let elementFinder: ElementFinder = element(By.css('.e-accordion'));
    ...
    it('Default', async (done: DoneFn) => {
        Helper.getAndWait(Helper.tsUrl + 'accordion/default.html', Helper.accordion);
    ...
    it('Ajax-content', async (done: DoneFn) => {
        Helper.getAndWait(Helper.tsUrl + 'accordion/ajax.html', Helper.accordion);
        await element(By.css('.e-accordion')).isDisplayed().then((flag: boolean) => {
            expect(flag).toEqual(true);
        });
        browser.sleep(1000);
    ...
    it('Icons', async (done: DoneFn) => {
        Helper.getAndWait(Helper.tsUrl + 'accordion/icon.html', Helper.accordion);
        await element(By.css('.e-accordion')).isDisplayed().then((flag: boolean) => {
            expect(flag).toEqual(true);
        });
        browser.sleep(1000);
