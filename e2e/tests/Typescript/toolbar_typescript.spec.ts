...
let elementFinder: ElementFinder = element(By.className('.e-sample-resize-container'));
    ...
    it('Default', async (done: DoneFn) => {
        Helper.getAndWait(Helper.tsUrl + 'toolbar/default.html', Helper.toolbar);
        browser.sleep(3000);
    ...
    it('Popup', async (done: DoneFn) => {
        Helper.getAndWait(Helper.tsUrl + 'toolbar/popup.html', Helper.toolbar);
        browser.sleep(1000);
        await element(By.css('.e-sample-resize-container')).isDisplayed().then((flag: boolean) => {
            expect(flag).toEqual(true);
        });
    ...
    it('Alignment', async (done: DoneFn) => {
        Helper.getAndWait(Helper.tsUrl + 'toolbar/alignment.html', Helper.toolbar);
        browser.sleep(1000);
        await element(By.css('.e-sample-resize-container')).isDisplayed().then((flag: boolean) => {
            expect(flag).toEqual(true);
        });
